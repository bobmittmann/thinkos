...
/* Maximum offset (can be any size < 2^31). Lower values give faster
   ...
   you. */
//#define LZ_MAX_OFFSET 8192
//unsigned int lz_max_offset  = 16384;
...
int LZ_Compress( unsigned char *in, unsigned char *out,
    unsigned int insize, unsigned int max_offset  )
{
    ...
    do
    {
        ...
        if( inpos > max_offset ) maxoffset = max_offset;
...
int LZ_CompressFast( unsigned char *in, unsigned char *out,
    unsigned int insize, unsigned int *work, unsigned int max_offset)
{
    unsigned char marker, symbol;
    ...
    do
    {
        ...
        while( (index != 0xffffffff) && ((inpos - index) < max_offset) )
        {
            /* Get pointer to candidate string */
            ptr2 = &in[ index ];
