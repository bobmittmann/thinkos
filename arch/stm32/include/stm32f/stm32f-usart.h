...
#define USART_RTOF (1 << 11) /* Receiver timeout flag */
...
Note: If a time equal to the value programmed in RTOR register separates 2 characters, RTOF is not set. If this time exceeds this value + 2 sample times (2/16 or 2/8, depending on the oversampling method), RTOF flag is set. The counter counts even if RE = 0 but RTOF is set only when RE = 1. If the timeout has already elapsed when RE is set, then RTOF will be set. If the USART does not support the Receiver timeout feature, this bit is reserved and
...
/* This bit is set and cleared by software. 
   ...
   1: An USART interrupt is generated when the RTOF bit is set in the USART_ISR register. Note: If the USART does not support the Receiver timeout feature, this bit is reserved and forced by hardware to ‘0’. */


/* Bits 25:21 DEAT[4:0]: Driver Enable assertion time */ 
#define USART_DEAT_MSK ((0x1f) << 21) 
#define USART_DEAT_SET(VAL) (((VAL) << 21) & USART_DEAT_MSK)
#define USART_DEAT_GET(REG) (((REG) & USART_DEAT_MSK) >> 21)
...
#define USART_DEDT_MSK ((0x1f) << 16) 
#define USART_DEDT_SET(VAL) (((VAL) << 16) & USART_DEDT_MSK)
#define USART_DEDT_GET(REG) (((REG) & USART_DEDT_MSK) >> 16)
/* This 5-bit value defines the time between the end of the last stop bit, in a transmitted message, and the de-activation of the DE (Driver Enable) signal. It is expressed in sample time units (1/8 or 1/16 bit duration, depending on the oversampling rate). If the USART_TDR register is written during the DEDT time, the new data is transmitted only when the DEDT and DEAT times have both elapsed.
This bit field can only be written when the USART is disabled (UE=0).
Note: If the Driver Enable feature is not supported, this bit is reserved and must be kept
cleared. */
...
#define STM32F_USART_RTOR 0x14


/* Bits 31:24 BLEN[7:0]: Block Length */
#define USART_BLEN_MSK ((0xff) << 24) 
#define USART_BLEN_SET(VAL) (((VAL) << 24) & USART_BLEN_MSK)
#define USART_BLEN_GET(REG) (((REG) & USART_BLEN_MSK) >> 24)

/* This bit-field gives the Block length in Smartcard T=1 Reception. Its value equals the number of information characters + the length of the Epilogue Field (1-LEC/2-CRC) - 1. Examples:
BLEN = 0 -> 0 information characters + LEC
BLEN = 1 -> 0 information characters + CRC
BLEN = 255 -> 254 information characters + CRC (total 256 characters))
In Smartcard mode, the Block length counter is reset when TXE=0. This bit-field can be used also in other modes. In this case, the Block length counter is reset when RE=0 (receiver disabled) and/or when the EOBCF bit is written to 1. Note: This value can be programmed after the start of the block reception (using the data from the LEN character in the Prologue Field). It must be programmed only once per received block. */


/* Bits 23:0 RTO[23:0]: Receiver timeout value */ 
#define USART_RTO_MSK ((0x00ffffff) << 0) 
#define USART_RTO_SET(VAL) (((VAL) << 0) & USART_RTO_MSK)
#define USART_RTO_GET(REG) (((REG) & USART_RTO_MSK) >> 0)
