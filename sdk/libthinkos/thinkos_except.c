...
void __xcpt_void_task(struct thinkos_except * xcpt)
{
	DCC_LOG(LOG_TRACE, "=============");
	for(;;);
}
...
//	if ((irq = __xcpt_next_active_irq(ipsr - 16)) >= 0) {
	...
	if (icsr & SCB_ICSR_RETTOBASE) {
		...
		sf->xpsr = 0x01000000;
		sf->pc = (uint32_t)__xcpt_void_task;
