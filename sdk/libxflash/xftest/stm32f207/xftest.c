...
extern int yflash(uint32_t blk_offs, unsigned int blk_size, 
				  const struct magic * magic);
...
int __attribute__((noreturn)) main(int argc, char ** argv)
{
	...
	cdc = usb_cdc_init(&stm32f_otg_fs_dev, cdc_acm_str, cdc_acm_strcnt);
	usb_cdc = cdc;

	thinkos_sleep(500);
	DCC_LOG(LOG_TRACE, "Xflash test...");
	...
	for (;;) {
		...
		case 'F':
			...
			yflash(0, 32 * 1024, &firmware_magic);
			break;
