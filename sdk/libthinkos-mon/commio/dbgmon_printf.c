...
#else
			buf[0] = c;
			cnt += monitor_comm_send(comm, buf, 1);
#endif
			continue;
		}
...
print_buf:
		...
		if (w > n) {
			...
			if (flags & ZERO) {
				...
				r = monitor_comm_send(comm, monitor_zeros, w - n);
			} else {
				r = monitor_comm_send(comm, monitor_blanks, w - n);
			}
			cnt += r;
		}
		...
		cnt += monitor_comm_send(comm, cp, n);
