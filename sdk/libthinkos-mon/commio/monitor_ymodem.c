...
#if 0
static int monitor_ymodem_rcv_cancel(struct monitor_comm * comm, struct ymodem_rcv * rx)
{
						...
						struct ymodem_rcv * rx)
{
	...
	for (;;) {
		...
		monitor_comm_send(comm, &rx->sync, 1);

		monitor_alarm(XMODEM_RCV_TMOUT_MS);

		for (;;) {
			int c;

			ret = monitor_comm_recv(comm, pkt, 1);
			if (ret < 0) {
		...
		while (rem) {
			monitor_alarm(XMODEM_RCV_TMOUT_MS);
			ret = monitor_comm_recv(comm, cp, rem);
			if (ret < 0) {
...
error:
		/* flush */
		while (monitor_comm_recv(comm, pkt, 1024) > 0);
		ret = -1;
		break;
