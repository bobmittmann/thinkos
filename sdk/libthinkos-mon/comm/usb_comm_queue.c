...
#define THINKOS_MONITOR_ENABLE_FLOWCTL 0
#endif
...
//	while ((n = (int32_t)(dev->rx_seq - ack)) == 0) {
	...
	if (monitor_is_set(MONITOR_COMM_RCV)) {
		DCC_LOG(LOG_WARNING, "monitor_is_set()!");
	}
...
const struct monitor_comm * usb_comm_queue_init(const usb_dev_t * usb)
{
