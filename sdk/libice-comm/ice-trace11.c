...
void ice_trace11(const struct dcc_trace_entry * __entry, int __a, 
				 ...
				 int __j, int __k)
{
	...
	} while ((16 - ((head - comm->tx_tail) & 0xffff)) < 12);
	...
	comm->tx_buf.u32[head++ & 0xf] = __k;
	comm->tx_head = head;
ret:
	cm3_faultmask_set(fm);  /* restore fault mask */
}

void dcclog11(const struct dcc_trace_entry *, int, 
			 int,  int, int, int,
			 int,  int, int, int,
			 int, int) 
	__attribute__ ((weak, alias ("ice_trace11")));
