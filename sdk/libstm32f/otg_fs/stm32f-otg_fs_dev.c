							   ...
							   void * buf, unsigned int len)
{
	...
	deptsiz = otg_fs->inep[idx].dieptsiz;
	...
	*/
	...
	otg_fs->inep[idx].diepctl = depctl | OTG_FS_EPENA | OTG_FS_CNAK; 
...
static void __ep0_tx_push(struct stm32f_otg_drv * drv)
{
	...
	if (ep->xfr_rem == 0) {
		DCC_LOG(LOG_ERROR, "/!\\ ep->xfr_rem == 0 /!\\");
		return;
	} 

	if (xfrsiz == 0) {
		DCC_LOG(LOG_ERROR, "/!\\ (xfrsiz == 0) /!\\");
		return;
	} 
	...
	DCC_LOG5(LOG_TRACE, "[0] mpsiz=%d pktcnt=%d xfrsiz=%d free=%d cnt=%d", 
			 mpsiz, pktcnt, xfrsiz, free, cnt);
...
//	pktcnt = (ep->xfr_rem + (mpsiz - 1)) / mpsiz;
//	xfrsiz = (pktcnt > 1) ? mpsiz : ep->xfr_rem;
...
static bool __ep_tx_push(struct stm32f_otg_drv * drv, int idx, int ep_id)
{
	...
	DCC_LOG5(LOG_TRACE, "ep_id=%d mpsiz=%d pktcnt=%d xfrsiz=%d free=%d", 
						   ...
						   void * xfr_buf, int buf_len)
{
	...
	if (ep_id == 0) {
		...
		if (info->addr & USB_ENDPOINT_IN) {
			...
			otg_fs->inep[idx].diepctl = depctl | OTG_FS_TXFNUM_SET(idx);
...
//			otg_fs->outep[1].doeptsiz = ep->doeptsiz;
//			otg_fs->outep[2].doeptsiz = ep->doeptsiz;
...
//			otg_fs->outep[1].doepctl = depctl | OTG_FS_EPENA | OTG_FS_CNAK;
//			otg_fs->outep[2].doepctl = depctl | OTG_FS_EPENA | OTG_FS_CNAK;
			otg_fs->outep[idx].doepctl = depctl | OTG_FS_EPENA | OTG_FS_CNAK;

			/* Enable endpoint interrupt */
			otg_fs->daintmsk |= OTG_FS_OEPM(idx);
	//		otg_fs->daintmsk |= (0xffff) << 16;
		}
...
static void stm32f_otg_dev_ep_out_done(struct stm32f_otg_drv * drv, 
									   int idx, int len)
{
	struct stm32f_otg_fs * otg_fs = STM32F_OTG_FS;
	struct stm32f_otg_ep * ep;
...
static void stm32f_otg_dev_ep_in(struct stm32f_otg_drv * drv, int idx)
{
	struct stm32f_otg_fs * otg_fs = STM32F_OTG_FS;
	struct stm32f_otg_ep * ep;
...
static void __ep0_tx_setup(struct stm32f_otg_drv * drv)
{
	struct stm32f_otg_ep * ep = &drv->ep[0];
	struct stm32f_otg_fs * otg_fs = STM32F_OTG_FS;
	uint32_t diepctl;
	...
	if (ep->xfr_rem < mpsiz) {
		pktcnt = 1;
		xfrsiz = ep->xfr_rem;
	} else if (ep->xfr_rem <= ep->xfr_max) {
		pktcnt = ep->xfr_rem / mpsiz;
		pktcnt = 1;
		xfrsiz = pktcnt * mpsiz;
	} else {
		pktcnt = ep->xfr_max / mpsiz;
		pktcnt = 1;
		xfrsiz = pktcnt * mpsiz;
	}

	ep->state = EP_IN_DATA;
	DCC_LOG3(LOG_TRACE, "pktcnt=%d xfrsiz=%d rem=%d", 
			 pktcnt, xfrsiz, ep->xfr_rem - xfrsiz);
...
static void __ep0_tx_done(struct stm32f_otg_drv * drv)
{
	...
	DCC_LOG(LOG_WARNING, "ep->xfr_rem != 0");
//		 __ep0_tx_setup(drv);
}
...
static void stm32f_otg_dev_ep0_out(struct stm32f_otg_drv * drv)
{
	...
	DCC_LOG(LOG_ERROR, "EP0 != EP_OUT_DATA_LAST");
}

#if 0
static void stm32f_otg_dev_ep0_setup_out(struct stm32f_otg_drv * drv)
{
	struct stm32f_otg_fs * otg_fs = STM32F_OTG_FS;
	struct usb_request * req = &drv->req;
	struct stm32f_otg_ep * ep = &drv->ep[0];
	...
	/* last and only transfer */
	if (ep->state != EP_OUT_DATA_LAST) {
		DCC_LOG(LOG_WARNING, "ep->state != EP_OUT_DATA_LAST");
		return;
	}
...
static void stm32f_otg_dev_ep0_setup(struct stm32f_otg_drv * drv)
{
	...
	if (req->length == 0) {
		...
		DCC_LOG(LOG_TRACE, "EP0 [SETUP] IN Dev->Host len=0");
	...
	if (req->type & 0x80) {
		...
		DCC_LOG1(LOG_TRACE, "EP0 [SETUP] IN Dev->Host (%d)", ep->xfr_rem);
		...
		if (ep->xfr_rem < ep->xfr_max) {
			...
			DCC_LOG1(LOG_TRACE, "EP0 [OUT_DATA_LAST] OUT Host->Dev len=%d!!!!",
					 req->length);
		} else {
			ep->state = EP_OUT_DATA;
			DCC_LOG1(LOG_TRACE, "EP0 [OUT_DATA] OUT Host->Dev len=%d", 
					 req->length);
		}
...
void stm32f_otg_fs_isr(void)
{
	...
	DCC_LOG1(LOG_TRACE, "GINTSTS=0x%08x", gintsts);
	...
	if (gintsts & OTG_FS_IEPINT) {
		...
		if (ep_intr & OTG_FS_IEPINT0) {
			...
			if (diepint & OTG_FS_XFRC) {
				...
				DCC_LOG(LOG_TRACE, "[0] <IEPINT> <IN XFRC>");
				__ep0_tx_done(drv);
		...
		if (ep_intr & OTG_FS_IEPINT1) {
			...
			if (diepint & OTG_FS_XFRC) {
				...
				__ep_tx_push(drv, 1, 1 + OTG_INEP_OFF);
			} 
		}
#endif
		...
		if (ep_intr & OTG_FS_IEPINT2) {
			...
			if (diepint & OTG_FS_TXFE) {
				...
				__ep_tx_push(drv, 2, 2 + OTG_INEP_OFF);
			}
			if (diepint & OTG_FS_XFRC) {
		...
		if (ep_intr & OTG_FS_IEPINT3) {
			...
			if (diepint & OTG_FS_TXFE) {
				...
				__ep_tx_push(drv, 3, 3 + OTG_INEP_OFF);
			}
			if (diepint & OTG_FS_XFRC) {
	...
	if (gintsts & OTG_FS_RXFLVL) {
		...
		if (epnum == 0) {
			...
			case OTG_FS_PKTSTS_SETUP_COMP:
				...
				DCC_LOG1(LOG_TRACE, "[%d] <RXFLVL> <SETUP_COMP>", epnum);
				break;
			case OTG_FS_PKTSTS_SETUP_UPDT:
				/* SETUP data packet received */
				DCC_LOG1(LOG_TRACE, "[%d] <RXFLVL> <SETUP_UPDT>", epnum);
...
//				otg_fs->gintmsk |=  OTG_FS_SOFM;
			...
			case OTG_FS_PKTSTS_OUT_XFER_COMP:
				DCC_LOG1(LOG_TRACE, "[%d] <RXFLVL> <OUT_XFER_COMP>", epnum);
				stm32f_otg_dev_ep_out_done(drv, epnum, len);
				break;
			}
		}
