...
function q31cos(x::Int32, poly::Int=2)
	LOG2_N::Int32 = Int32(log2(N_SIN))
	MASK_N::Int32 = (1 << LOG2_N) - 1
	DX::Int32 = 1 << (31 - LOG2_N)
	Q31_ONE = Int32(Q31(1.0) - 1)
...
#	+90dg 
	x0 = x0 + Int32(Q31(0.5))
	i = x0 >>> (31 - LOG2_N)
	cos_x0 = qsintab[(i & MASK_N) + 1] * ((x0 < 0) ? -1 : 1)
...
#			Q31DIV(Q31MUL(Q31DIV(Q31MUL(qx1, dx2), (DX + DX)), dy321), DX))
	...
	if (poly == 1)
		...
		x1 = x0 + DX
		cos_x1 = qsintab[((i + 1) & MASK_N) + 1] * ((x1 < 0) ? -1 : 1)
	...
	elseif (poly == 2)
		...
		x2 = x1 + DX
		cos_x2 = qsintab[((i + 2) & MASK_N) + 1] * ((x2 < 0) ? -1 : 1)
...
function mk_c_q31cos_trig(n::Int64)
	txt = AbstractString[
	""
	"#if (FIXPT_SINCOS_INTRPL_TRIG)"
	"/* Circular interpolation version, using the trigonometric "
	"   identity: cos(a + b) = sin(a)*cos(b) + cos(a)*sin(b) "
	"*/"
	"int32_t q31sin(int32_t x)"
	"{"
	"\tint32_t x0;"
