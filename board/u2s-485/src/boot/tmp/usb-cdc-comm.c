...
static const struct cdc_acm_descriptor_set comm_usb_cdc_acm_desc_cfg = {
	.cfg = {
...
const struct usb_descriptor_string comm_usb_cdc_language_english_us = {
	4, USB_DESCRIPTOR_STRING, { 0x0409 }
};

const struct usb_descriptor_string comm_usb_cdc_stmicro_str = {
...
/* Interface 0: "ST VCOM" */
const struct usb_descriptor_string comm_usb_cdc_st_vcom_str = {
	8 * 2 + 4, USB_DESCRIPTOR_STRING, {
		'S', 'T', ' ', 'V', 'C', 'O', 'M', 0
	}
};

const struct usb_descriptor_string comm_usb_cdc_thinkos_str = {
	8 * 2 + 4, USB_DESCRIPTOR_STRING, {
		'T', 'h', 'i', 'n', 'k', 'O', 'S', 0
	}
};

const struct usb_descriptor_string comm_usb_cdc_serial_num_str = {
...
static const struct usb_descriptor_string * const comm_usb_cdc_acm_str[] = {
	&comm_usb_cdc_language_english_us,
	&comm_usb_cdc_stmicro_str,
	&comm_usb_cdc_thinkos_str,
	&comm_usb_cdc_serial_num_str,
	&comm_usb_cdc_st_vcom_str 
};

#define USB_STRCNT() (sizeof(comm_usb_cdc_acm_str) / sizeof(uint8_t *))

static const struct cdc_line_coding comm_usb_cdc_usb_cdc_lc = {
    .dwDTERate = 38400,
    .bCharFormat = 0,
    .bParityType = 0,
    .bDataBits = 8
...
static const uint16_t comm_usb_cdc_device_status = 
	USB_DEVICE_STATUS_SELF_POWERED;
static const uint16_t comm_usb_cdc_interface_status = 0x0000;
#endif
...
int comm_usb_cdc_enqueue(struct usb_cdc_acm_dev * dev)
{
	uint32_t seq;
	uint32_t ack;
	int free;
	...
	if ((free = CDC_RX_BUF_SIZE - (int32_t)(seq - ack)) > 0) {
		...
		if ((int32_t)(seq - ack) == CDC_RX_BUF_SIZE) {
			DCC_LOG(LOG_INFO, VT_PSH VT_FCY VT_REV " fifo full RX PAUSE " VT_POP);
			dev->rx_paused = true;
		}
...
int comm_usb_cdc_dequeue(struct thinkos_comm_drv * drv,
						   void * buf, unsigned int len)
{
	...
	if (pos == 0) {
		DCC_LOG4(LOG_INFO, "1. n=%d ack=%d pos=%d cnt=%d...", 
				 n, ack, pos, cnt);
		__thinkos_memcpy(dst, dev->rx_q.buf, cnt);
	} else {
		unsigned int m = CDC_RX_BUF_SIZE - pos;
		unsigned int l;

		m = MIN(m, cnt);
		__thinkos_memcpy(dst, &dev->rx_q.buf[pos], m);
		dst += m;

		l = cnt - m;
		__thinkos_memcpy(dst, dev->rx_q.buf, l);
...
#if 0
	if (dev->rx_paused && ((seq - ack) < (CDC_RX_BUF_SIZE / 2))) {
		DCC_LOG(LOG_INFO, VT_PSH VT_FCY VT_REV " RX RESUME " VT_POP);
		dev->rx_paused = false;
		/* receive more data into buffer */
		comm_usb_cdc_enqueue(dev);
	}

	if ((int32_t)(seq - ack) > 0) {
...
static void usb_cdc_comm_ep_on_rcv(usb_class_t * cl, 
						   unsigned int ep_id, unsigned int len)
{
	struct usb_cdc_acm_dev * dev = &cl->dev;

	DCC_LOG1(LOG_MSG, VT_PSH VT_FCY VT_REV 
			 "IRQ pkt_len=%d COMM_RCV..." VT_POP, len);

//	comm_usb_cdc_enqueue(dev);

	thinkos_krn_comm_wakeup_head(dev->krn, dev->rx_wq);
}

static void usb_cdc_comm_ep_on_eot(usb_class_t * cl, unsigned int ep_id)
{
	struct usb_cdc_acm_dev * dev = &cl->dev;

	DCC_LOG1(LOG_MSG, "ON EOT comm=%d", dev->tx_wq);
	thinkos_krn_comm_wakeup_head(dev->krn, dev->tx_wq);
}

static void usb_cdc_comm_ep_on_eot_int(usb_class_t * cl, unsigned int ep_id)
{
	DCC_LOG1(LOG_INFO, "ep_id=%d", ep_id);
}

static const usb_dev_ep_info_t comm_usb_cdc_in_info = {
	.addr = USB_ENDPOINT_IN + EP_IN0_ADDR,
	.attr = ENDPOINT_TYPE_BULK,
	.mxpktsz = CDC_EP_IN_MAX_PKT_SIZE,
	.on_in = usb_cdc_comm_ep_on_eot
};

static const usb_dev_ep_info_t comm_usb_cdc_out_info = {
	.addr = USB_ENDPOINT_OUT + EP_OUT0_ADDR,
	.attr = ENDPOINT_TYPE_BULK,
	.mxpktsz = CDC_EP_OUT_MAX_PKT_SIZE,
	.on_out = usb_cdc_comm_ep_on_rcv
};

static const usb_dev_ep_info_t comm_usb_cdc_int_info = {
	.addr = USB_ENDPOINT_IN + EP_INT0_ADDR,
	.attr = ENDPOINT_TYPE_INTERRUPT,
	.mxpktsz = CDC_EP_INT_MAX_PKT_SIZE,
	.on_in = usb_cdc_comm_ep_on_eot_int
};


/* Setup requests callback handler */
static int comm_usb_cdc_on_setup(usb_class_t * cl, 
								struct usb_request * req, void ** ptr) 
{
	...
	case STD_GET_DESCRIPTOR:
		...
		if (desc == USB_DESCRIPTOR_CONFIGURATION) {
			...
			*ptr = (void *)&comm_usb_cdc_acm_desc_cfg;
		...
		if (desc == USB_DESCRIPTOR_STRING) {
			...
			if (n < USB_STRCNT()) {
				*ptr = (void *)comm_usb_cdc_acm_str[n];
				len = comm_usb_cdc_acm_str[n]->length;
			}
			break;
		}
		len = 0;
	...
	case STD_SET_CONFIGURATION: {
		...
		if (value) {
			...
			dev[0].in_ep = usb_dev_ep_init(usb, &comm_usb_cdc_in_info, 
										   NULL, 0);
			dev[0].out_ep = usb_dev_ep_init(usb, &comm_usb_cdc_out_info, 
											NULL, 0);
			dev[0].int_ep = usb_dev_ep_init(usb, &comm_usb_cdc_int_info, 
											NULL, 0);
			dev->configured = 1;
		} else {
	...
	case STD_GET_STATUS_DEVICE:
		...
		*ptr = (void *)&comm_usb_cdc_device_status;
		len = 2;
		break;
#endif
	...
	case STD_GET_STATUS_INTERFACE:
		...
		*ptr = (void *)&comm_usb_cdc_interface_status;
		len = 2;
		break;
#endif
	...
	case GET_LINE_CODING:
		...
		/* Return Line Coding */
		*ptr = (void *)&comm_usb_cdc_usb_cdc_lc;
...
static const usb_dev_ep_info_t comm_usb_cdc_ep0_info = {
	.addr = 0,
	.attr = ENDPOINT_TYPE_CONTROL,
	.mxpktsz = EP0_MAX_PKT_SIZE,
	.on_setup = comm_usb_cdc_on_setup
};

static void comm_usb_cdc_on_reset(usb_class_t * cl)
{
	struct usb_cdc_acm_dev * dev = &cl->dev;
	...
	DCC_LOG1(LOG_INFO, "ctl ep=%d...", dev->ctl_ep);
}

static void comm_usb_cdc_on_suspend(usb_class_t * cl)
{
	struct usb_cdc_acm_dev * dev = &cl->dev;
	DCC_LOG(LOG_TRACE, "...");
	dev->status = 0;
...
static void comm_usb_cdc_on_wakeup(usb_class_t * cl)
{
	DCC_LOG(LOG_TRACE, "...");
}

static void comm_usb_cdc_on_error(usb_class_t * cl, int code)
{
	DCC_LOG(LOG_TRACE, "...");

}

static void comm_usb_cdc_on_sof(usb_class_t * cl)
{
//	struct usb_cdc_acm_dev * dev = &cl->dv;
	DCC_LOG(LOG_TRACE, "...");
}

int usb_cdc_comm_send(struct thinkos_comm_drv * drv, 
						   const void * buf, unsigned int len)
{
	...
	if (dev->configured) {
		ret = usb_dev_ep_pkt_xmit(dev->usb, dev->in_ep, buf, len);
		DCC_LOG2(LOG_INFO, "len=%d ret=%d", len, ret);
	} else {
		DCC_LOG1(LOG_INFO, "unconfigured, discarding %d bytes...", len);
		ret = len;
	}
...
int usb_cdc_comm_recv(struct thinkos_comm_drv * drv,
						   void * buf, unsigned int len)
{
	struct usb_cdc_acm_dev * dev = &drv->dev;
	int cnt;
	...
	if (dev->configured) {
		cnt = usb_dev_ep_pkt_recv(dev->usb, dev->out_ep, buf, len);
	} else {
		cnt = 0;
	}
	return cnt;
...
static int usb_cdc_comm_ctrl(struct thinkos_comm_drv * drv, unsigned int opc)
{
...
void usb_cdc_comm_signal(struct thinkos_comm_drv * drv, unsigned int sig)
{
//	struct usb_cdc_acm_dev * dev = &drv->dev;

	if (sig == COMM_TX_PEND) {
//		if (dev->configured) {
			DCC_LOG(LOG_TRACE, "COMM TX PEND...");
...
int usb_cdc_comm_open(struct thinkos_comm_drv * drv)
{
	struct usb_cdc_acm_dev * dev = &drv->dev;

	if (dev->configured) {
		usb_dev_ep_ctl(dev->usb, dev->in_ep, USB_EP_NAK_CLR);
		usb_dev_ep_ctl(dev->usb, dev->out_ep, USB_EP_NAK_CLR);
	}

	return THINKOS_OK;
}

int usb_cdc_comm_close(struct thinkos_comm_drv * drv)
{
	struct usb_cdc_acm_dev * dev = &drv->dev;

	if (dev->configured) {
		usb_dev_ep_ctl(dev->usb, dev->in_ep, USB_EP_NAK_SET);
		usb_dev_ep_ctl(dev->usb, dev->out_ep, USB_EP_NAK_SET);
	}
	return 0;
}


static const usb_class_events_t comm_usb_cdc_ev = {
	.on_reset = comm_usb_cdc_on_reset,
	.on_suspend = comm_usb_cdc_on_suspend,
	.on_wakeup = comm_usb_cdc_on_wakeup,
	.on_error = comm_usb_cdc_on_error,
...
{
	struct usb_cdc_acm_dev * dev = &drv->dev;
	struct usb_class_if * cl = (struct usb_class_if *)dev;
	int ret;

	/* initialize USB device */
	dev->krn = krn;
	dev->usb = (usb_dev_t *)usb_drv;
	dev->rx_q.seq = 0;
	dev->rx_q.ack = 0;
	dev->status = 0;
	dev->tx_wq = tx_wq;
	...
	ret = usb_dev_init(dev->usb, cl, &comm_usb_cdc_ev);
	(void)ret;
	DCC_LOG1(LOG_INFO, "usb_dev_init() --> %d", ret);

	return ret;
}
void usb_cdc_comm_reset(struct thinkos_comm_drv * drv, int priority)
{
	DCC_LOG1(LOG_TRACE, "priority=%d", priority);
}

const struct thinkos_comm_drv_op usb_cdc_comm_drv_op  = {
	.open = usb_cdc_comm_open,
	.send = usb_cdc_comm_send,
	.recv = usb_cdc_comm_recv,
	.close = usb_cdc_comm_close,
	.ctrl = usb_cdc_comm_ctrl,
	.signal = usb_cdc_comm_signal,
	.init = usb_cdc_comm_init,
	.reset = usb_cdc_comm_reset
};

struct thinkos_comm_drv usb_cdc_drv_instance;

const struct thinkos_comm usb_cdc_comm_instance = {
	.tag = "USBCDC0",
	.drv = &usb_cdc_drv_instance,
	.drv_op = &usb_cdc_comm_drv_op
};
